...
for (file in files) {
  ...
  frame["order"] <- stringi::stri_sub(file, 14,16)
  frames[[file]] <- frame
}
...
study_data <- study_data %>%
  ...
  mutate(
    ...
    KAGJustOt = get_value(c_across(matches("H.KnAG...Ot$"))),
    KSTJustOt = get_value(c_across(matches("H.KnAG...Ot$"))),
    KPRJustOt = get_value(c_across(matches("H.KnAG...Ot$")))
  )
```
...
```{r}
as.data.frame(study_data_long) %>% 
  group_by(form) %>% 
  rstatix::pairwise_t_test(Kc ~ order) %>% 
  flextable() %>%
  colformat_double(digits = 2) %>%
  fit_to_width(11.5)
```
...
# Perform a mixed-design ANOVA.
# DV: Kc (knowledge attribution)
# Between-subjects IV: condition (Harm/Help), order
# Within-subjects IV: form (ST/CA/AG)
results <- ezANOVA(data = study_data_long,
        dv = Kc,
        between = .(condition, order) ,
        within = .(form),
        wid = participant
        )$ANOVA
